...
CROP_LENGTH=90             # Cut reads to this length after filtering. Just under amplicon length
...
MIN_HIT_LENGTH=40          # Minimum vsearch hit length to keep in results (int, 1+)

# Filters
MIN_HITS_SAMPLE=3          # Minimum number of hits in at least one sample  to keep in results (int, 1+)
MIN_HITS_EXPERIMENT=5      # Minimum number of hits in whole experiment to keep in results (int, 1+)

# Non-annotated reads
NUM_NON_ANNOTATED_SEQ=500   # Number of unique most-frequent non-annotated reads to keep (int, 1+)

# Multiple hits
MIN_DEPTH_MULTI=3          # Min depth to report unique reads per sample in multiple hit reports

# OTUs
SKIP_OTUS=0                 # 1 to skip OTU creation, 0 to use it
MIN_SIZE_FOR_OTU=5         # Only unique reads with at least this coverage will be used for OTUs
